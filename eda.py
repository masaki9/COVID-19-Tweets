...
def eda(data):
    ''' Plot charts for exploratory data analysis. '''

    df = pd.read_csv(data, header=0, sep=',')
    print('Dataset Size: {}'.format(df.shape))

    # Ensure the data only contains unicode strings.
    df['text_processed'] = df['text_processed'].values.astype('U')
    ...
    plt.ylabel('Number of Tweets', size=14)
    plt.title('Number of COVID-19 Vaccine Related Tweets by Month', size=18)
    ax = plt.gca()
    utils.add_bar_value_labels(ax)
    # plt.show()
    ...
    plt.title('Common Words in COVID-19 Vaccine Related Tweets', size=18)
    plt.axis('off')
